import { format } from "date-fns";
import { vi, enUS } from "date-fns/locale";
import { getPostServices } from "@/services/post.service";
import Image from "next/image";
import NotFound from "@/components/not-found";
import LatestPosts from "../../components/latestPosts";
import { Separator } from "@/components/ui/separator";
import DomParser from "@/components/domParser";

export async function generateMetadata({
  params,
}: {
  params: Promise<{ id: string; locale: string }>;
}) {
  const locale = (await params).locale;
  const postID = (await params).id;

  const {
    payload: { data },
  } = await getPostServices.getPost("evolving-research", postID, locale);

  return {
    title: data.metadata.title,
    // description: data.metadata.description,
    openGraph: {
      title: data.metadata.title,
      // description: data.metadata.description,
      images: [{ url: data.metadata.thumbnail }],
      url: `https://iscm.ueh.edu.vn/${locale}/expert_insight/${data.metadata.slug}`,
      type: "article",
      locale: locale,
      article: {
        publishedTime: data.metadata.publishDate,
        authors: ["https://iscm.ueh.edu.vn"],
        section: "Expert Insight",
        // tags: data.metadata.tags || [], // nếu có
      },
    },
  };
}

export default async function page({
  params,
}: {
  params: Promise<{ id: string; locale: string }>;
}) {
  const locale = (await params).locale;
  const postID = (await params).id;

  try {
    const {
      payload: { data },
    } = await getPostServices.getPost("evolving-research", postID, locale);

    return (
      <section className="section-sm mt-3">
        <div className="container">
          <div className="row">
            <section className="col-lg-8 col-12 content-body">
              <div className="row">
                <div className="col-12">
                  <h1 className="section-title mb-2 text-2xl">
                    {data.metadata.title}
                  </h1>
                </div>
              </div>

              <div className="row mb-8">
                <div className="col-6">
                  <em className="text-[15px] flex items-end h-full">
                    {format(
                      data.metadata.publishDate,
                      locale === "en"
                        ? "dd LLL, yyyy"
                        : "dd 'tháng' MM 'năm' yyyy",
                      {
                        locale: locale === "en" ? enUS : vi,
                      }
                    )}
                  </em>
                </div>
                <div className="col-6">
                  <div className="flex justify-end gap-3">
                    {data.metadata.sdgs.map((sdg) => (
                      <div key={sdg}>
                        <Image
                          style={{ objectFit: "contain" }}
                          src={`/images/sdgs_icon/goal_${sdg}.svg`}
                          className="img-fluid w-9 h-9"
                          alt="thumbnail"
                          width={20}
                          height={20}
                          quality={90}
                        />
                      </div>
                    ))}
                  </div>
                </div>
              </div>

              <div className="row">
                <div className="col-12 mb-3 content">
                  <DomParser content={data.content} />
                </div>
                {data.metadata.author === "ISCM" && (
                  <>
                    <Separator className="mb-3" />
                    <div className="col-12 mb-50 text-right">
                      <em>
                        <b>
                          {locale === "en"
                            ? "News, photos: Institute of Smart City And Management"
                            : "Tin, ảnh: Viện Đô thị Thông minh và Quản lý"}
                        </b>
                      </em>
                    </div>
                  </>
                )}
              </div>
            </section>

            <div className="col-lg-4 col-12">
              <LatestPosts />
            </div>
          </div>
        </div>
      </section>
    );
  } catch (error) {
    console.log(error);
    return <NotFound />;
  }
}
