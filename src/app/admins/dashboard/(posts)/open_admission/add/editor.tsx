...
export default function Editor({ locale }: { locale: string }) {
  ...
  async function createPost(
    ...
    try {
      ...
      meta.thumbnail = `/images/open-admission/${folderName}/${thumb.name}`;
      ...
      // Upload Images
      await uploadImageService("open-admission", folderName, imageUploads);
      ...
      if (isPostPortal) {
        ...
        portal.local_cate = "open_admission";
      ...
      console.log(error);
      const _e = JSON.parse(JSON.stringify(error));

      console.log(_e);

      toast({
        title: _e.payload.message,
        duration: 1000,
      });
    }
  }
